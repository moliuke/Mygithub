...
#include "../../include/Dev_tcpserver.h"
//#include "UG_Lstparse.h"
//#include "UG_display.h"
...
static int UG_send(int fd,uint8_t *tx_buf,int tx_len)
{
	int i = 0;
	int ret = -1;
	int offset = 0;
...
int UG_NetSendback(user_t *user,uint8_t *tx_buf,int tx_len)
{
	...
	if(user->type == type_net)
	{
		...
		return UG_send(user->fd,tx_buf,tx_len);
	}
	else
	{
...
int UG_recv_process(void *arg)
{
	...
	if(ret == ERR_SOCKET_SHUTDOWN)
	{
		...
		pthread_mutex_unlock(&user_mutex);	
		ug_net_debug_printf("user has exit\n");
		if(user != NULL)
			free(user);
		user = NULL;
		return 0;
	...
	pthread_mutex_unlock(&user_mutex);
	ug_net_debug_printf("Cuser.ip = %s-------------user->ip = %s---------\r\n",Cuser.ip,user->ip);
